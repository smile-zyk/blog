...
&若 \lim \frac{\beta}{\alpha}=c\neq0,那么就说\beta是\alpha的同阶无穷小,记作 \beta=O(\alpha)\\
...
## 复合函数求极限法则
设函数 $y=f[g(x)]$ 是由函数 $u=g(x)$ 与函数 $y=f(u)$ 复合而成,$f[g(x)]$ 在点 $x_0$ 的某去心领域内由定义,若 $\lim\limits_{x \to x_0}g(x)=u_0$ , $\lim\limits_{u \to u_0}f(u)=A$,**且存在 $\delta_0>0$,当 $x \in \mathring{U}(x_0, \delta_0)$ 时,有 $g(x)\neq u_0$** ,则:
$$
\lim\limits_{x \to x_0}f[g(x)]=\lim\limits_{u \to u_0} f(u)=A
$$
> 为什么一定要有:存在 $\delta_0>0$,当 $x \in \mathring{U}(x_0, \delta_0)$ 时,有 $g(x)\neq u_0$.这一条件?
> 因为根据极限的定义,$x \to x_0$ 隐含了 $x \neq x_0$ 这一条件,在复合函数中,内层函数$g(x)$实际上变成了外层函数的"自变量",由于定义的限制,我们必须要保证,在 $x \to x_0$ 的过程中,新的自变量 $u=g(x)$ 在去心领域内 $\to u_0$ 的同时,不等于 $u_0$,这样才能保证 $f[g(x)]$ 的极限一定存在.
# 极限存在准则及两个重要极限
## 准则零
$\lim\limits_{x \to \infty}f(x)=A \iff \lim\limits_{x \to +\infty}f(x)=\lim\limits_{x \to - \infty}f(x)=A$ 
$\lim\limits_{x \to x_0}f(x)=A \iff \lim\limits_{x \to x_0^+}f(x)=\lim\limits_{x \to x_0^-}f(x)=A$ 
